...
因此，ClientHandler在处理请求的环节要对这个请求路径做分支判定，如果请求路径是这个值就要处理注册，
而不是去webapps目录下找到对应的文件
...
2:添加一个分支判断，如果请求路径是"/myweb/regUser"则执行处理注册的操作
  否则才执行原来处理环节操作(去webapps下找文件...)
3:处理请求的环节我们也做功能的拆分，不在ClientHandler中直接处理
  3.1:新建一个包com.webser.servlet
      这个包保存所有处理业务的类
